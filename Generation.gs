...
function generate() {
  ...
  try {
    ...
    var template = getFileByNameInFolder('_Xtemplate', folder); // get Gdocs '_template'
    
    var ts_begin = new Date().getTime(); // Number of ms since Jan 1, 1970
    
    deleteFilesByNameInFolder('_Xgenerated', folder); // delete all Gdocs '_generated' existing
    
    // create a copy of Gdocs '_template' and rename to '_generated'
    var file = template.makeCopy().setName('_Xgenerated');
    var generated = DocumentApp.openById(file.getId()); // get Gdocs '_generated' DocumentApp

    
    // Replace scopes
    complete_all_info(generated);
...
function complete_all_info(doc){

  var sheet_info = getSheetByName("Info");//Feuille "Info"
  var rows_info = sheet_info.getRange("B2:E4").getValues();//Plage comportant infos sur les parties prenantes et l'entreprise
  var rows_years = sheet_info.getRange("B8:C9").getValues();//Plage comportant des infos sur le années
  var footer=doc.getFooter();

  //=== Noms des parties prenantes ===
  complete_info(doc,"univ_tut",concatOrReplaceIfEmpty(rows_info[1][0],rows_info[1][1]," ","..."));//Tuteur universitaire
  complete_info(doc,"comp_tut",concatOrReplaceIfEmpty(rows_info[2][0],rows_info[2][1]," ","..."));//Tuteur industriel
  complete_info(doc,"stud_name",concatOrReplaceIfEmpty(rows_info[0][0],rows_info[0][1]," ","..."));//NOM Prénom sur la page garde
  complete_info(footer,"stud_name",rows_info[0][0]+" "+rows_info[0][1]);//NOM Prénom sur les pieds de pages
  complete_info(doc,"intro_stud_name",concatOrReplaceIfEmpty(rows_info[0][1],rows_info[0][0]," ","[Prénom NOM]"));//Prénom NOM dans l'intro

  //=== A propos de l'entreprise ===
  complete_info(doc,"dept",replaceIfEmpty(rows_info[0][3],"[département]"));//Département dans l'entreprise
  complete_info(doc,"comp_name",replaceIfEmpty(rows_info[1][3],"[Entreprise]"));//Nom entreprise
  complete_info(doc,"city",replaceIfEmpty(rows_info[2][3],"[Ville]"));//Ville entreprise

  //=== Années ===
  //--- Promotion  ---
  complete_info(doc,"prom_yr",concatOrReplaceIfEmpty(rows_years[0][0],rows_years[0][1]," - ","20... - 20..."));
  complete_info(footer,"prom_yr",concatOrReplaceIfEmpty(rows_years[0][0],rows_years[0][1]," - ","20... - 20..."));
  
  //--- Années en cours  ---
  complete_info(doc,"curr_yr",concatOrReplaceIfEmpty(rows_years[1][0],rows_years[1][1]," - ","20... - 20..."));
  

}

function concatOrReplaceIfEmpty(v1,v2,center,rep){
  if((v1=="")||(v2=="")){
    return rep;
  }
  else{
...
function complete_info(doc,k,v){
  doc.replaceText("{{"+k+"}}",v);
}
